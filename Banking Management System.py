#Welcome
#This is a Banking Management System Code. This code requires MySQL 8.0 or above to work.
...
def quit1():
    ...
    print("+","-"*53,"+",sep="")
def quit2():
    print("System will terminate shortly...")
    f=open("Logs.txt","a")
    lemon=datetime.datetime.now()
    apple=str(lemon)+": System Failed to Start | MySQL Connector not working\n"
    f.write(apple)
    guava="-"*60
...
def quit3():
    ...
    f=open("Logs.txt","a")
    lemon=datetime.datetime.now()
    apple=str(lemon)+": System Failed to Start | MySQL Server 8.0 not installed\n"
    f.write(apple)
    guava="-"*60
    apple=str(lemon)+": System Powered Off\n"+guava+"\n"
    f.write(apple)
...
def numbercheckPIN(a):
    ...
    return a
def namcheck(nam):
    while len(nam)>15 or nam=='' or nam.isspace()==True or " " in nam:
        if len(nam)>15:
            nam=input("Account Name cannot be more then 15 characters\nRetype: ")
        if nam=='':
            nam=input("Please Enter an Username: ")
        if nam.isspace()==True:
            nam=input("Cannot assign Spaces as Username: ")
        if " " in nam:
            nam=input("Spaces are not allowed in Username, Retype: ")
    return nam
...
def tupledisplay(t): #Displaying Table Content (For Admin Mode)
    ...
    L=[0,2,p-1]
    V1,V2,V3,V4=[],[],[],[]
    mycursor.execute("select mainbal from acc")
    W1=mycursor.fetchall()
    for g in W1:
        V1.append(g[0])
    mycursor.execute("select savings from acc")
    W2=mycursor.fetchall()
    for g1 in W2:
        V2.append(g1[0])
    mycursor.execute("select deptotal from acc")
    W3=mycursor.fetchall()
    for g2 in W3:
        V3.append(g2[0])
    mycursor.execute("select withtotal from acc")
    W4=mycursor.fetchall()
    for g3 in W4:
        V4.append(g3[0])
    A1=listlarge(V1)
    A2=listlarge(V2)
    ...
    if A4>8:
        ...
        B4=B3+10
    L1=[0,4,20,30,B1,B2,B3,B4]
    L2=[1,5,21,31,B1+1,B2+1,B3+1]
    L3=[4,20,30,B1,B2,B3,B4]
...
def login(main):
    ...
    if j[0]==passw: #Successful Login
        ...
        att=att-1
        if att==0:
            print("Too many incorrect attempts. Please wait for 10 seconds.") #Penalty Wait Time
            time.sleep(10)
            att=4
        else:
...
print("This program requires MySQL along with the connector to be installed.\nWe need your permission. Please provide the details.")
print()
time.sleep(0.5)
ans=input("Does this device have MySQL and the connector installed? (y/n): ")
ans=ans.lower()
...
if ans not in ["y","yes"]: #If MySQL is not installed
    print()
    print("Sorry but you need to have MySQL Server 8.0 with Python connector to run this program.")
...
try:
    ...
    print("We are unable to connect to MySQL database.\nPlease make sure the connector is working.")
    quit2()
def mysqlconn():
    try:
        global mydb
...
for i in mycursor:
    pass
if 'acc' not in i:
    mycursor.execute("create table acc(accno INTEGER PRIMARY KEY,name varchar(15),password varchar(10),mainbal varchar(1000) DEFAULT '0',savings varchar(1000) DEFAULT '0',deptotal varchar(1000) DEFAULT '0',withtotal varchar(1000) DEFAULT '0')")
main=''
